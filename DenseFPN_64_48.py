...
class DenseFPN():
    ...
    def __build(self):
        ...
        self.num_block = {'1':6,'2':8,'3':8,'4':8}
        self.growth_rate = {'1':48,'2':48,'3':48,'4':48}
        ...
        with tf.variable_scope('zsc_feature'):
            ...
            x = DenseBlock('DenseBlock_0',x,self.num_block['1'],self.growth_rate['1'],norm=self.norm,activate=self.activate,is_training=self.is_training)
            ...
            x = DenseBlock('DenseBlock_1',x,self.num_block['2'],self.growth_rate['2'],norm=self.norm,activate=self.activate,is_training=self.is_training)
            ...
            x = DenseBlock('DenseBlock_2',x,self.num_block['3'],self.growth_rate['3'],norm=self.norm,activate=self.activate,is_training=self.is_training)
            ...
            x = DenseBlock('DenseBlock_3',x,self.num_block['4'],self.growth_rate['4'],norm=self.norm,activate=self.activate,is_training=self.is_training)
    ...
    def __init__output(self):
        with tf.variable_scope('output'):
            ...
            if Detection_or_Classifier=='classifier':
                ...
                self.loss_yolo_1,class_loss_1,recall50_1,recall75_1,class_acc_1,avg_obj_1,avg_noobj_1,count_1,count_noobj_1 = self.loss([self.input_image, self.pred_yolo_1, self.true_yolo_1, self.true_boxes],
                                              self.anchors[:,20:], [1*num for num in self.max_grid], self.ignore_thresh)
                self.loss_yolo_2,class_loss_2,recall50_2,recall75_2,class_acc_2,avg_obj_2,avg_noobj_2,count_2,count_noobj_2 = self.loss([self.input_image, self.pred_yolo_2, self.true_yolo_2, self.true_boxes],
                                              self.anchors[:,10:20], [1*num for num in self.max_grid], self.ignore_thresh)
                self.loss_yolo_3,class_loss_3,recall50_3,recall75_3,class_acc_3,avg_obj_3,avg_noobj_3,count_3,count_noobj_3 = self.loss([self.input_image, self.pred_yolo_3, self.true_yolo_3, self.true_boxes],
                                              self.anchors[:,:10], [1*num for num in self.max_grid], self.ignore_thresh)
                self.all_loss = tf.reduce_mean(tf.sqrt(self.loss_yolo_1+self.loss_yolo_2+self.loss_yolo_3)) + \
                                tf.reduce_mean(class_loss_1+class_loss_2+class_loss_3) + regularzation_loss
    ...
    def loss(self,x,anchors, max_grid,ignore_thresh):
        ...
        _anchors = anchors/net_factor
        anchors_min_areas = _anchors[:,:,:,0,0]*_anchors[:,:,:,0,1]#none,1,1
        anchors_min_areas = tf.expand_dims(anchors_min_areas,3)#none,1,1,1
        anchors_max_areas = _anchors[:,:,:,-1,0]*_anchors[:,:,:,-1,1]#none,1,1
        anchors_max_areas = tf.expand_dims(anchors_max_areas,3)#none,1,1,1
        ...
        true_box_class = tf.argmax(y_true[..., 5:], -1)   #       (none,grid_h,grid_w,3,num_classes)
        ...
        snip_mask = tf.cast(true_areas>tf.expand_dims(anchors_min_areas,4)*2/3,
                            tf.float32)*tf.cast(true_areas<tf.expand_dims(anchors_max_areas,4)*1.5,tf.float32)#none,1,1,1,max_box_per_image
        true_areas = true_areas*snip_mask
        ...
        anchors_min_areas = tf.tile(anchors_min_areas,[1,grid_h,grid_w,self.num_anchors])
        anchors_max_areas = tf.tile(anchors_max_areas,[1,grid_h,grid_w,self.num_anchors])
        snip_mask = tf.cast(true_areas>anchors_min_areas*2/3,
                            tf.float32)*tf.cast(true_areas<anchors_max_areas*1.5,tf.float32)#none,grid_h,grid_w,3
        snip_mask = tf.expand_dims(snip_mask,-1)
        object_mask = object_mask*snip_mask#none,grid_h,grid_w,3
        ...
        xy_delta    = 1*xywh_mask   * (pred_box_xy-true_box_xy) * xywh_scale                          #(none,grid_h,grid_w,3,2) 计算唯一标记框的预测和真值xy偏差
        wh_delta    = 1*xywh_mask   * (pred_box_wh-true_box_wh) * xywh_scale                          #(none,grid_h,grid_w,3,2) 计算唯一标记框的预测和真值wh偏差
        ...
        #class_delta = 1*object_mask * tf.expand_dims(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=true_box_class, logits=pred_box_class), 4)
        ...
        class_delta = 10*object_mask*tf.reshape(class_delta,tf.concat([tf.shape(pred_box_class)[:-1],[-1]],axis=-1))
        ...
        loss = tf.reduce_sum(tf.square(xy_delta),       list(range(1,5))) + \
               ...
               tf.reduce_sum(tf.square(conf_delta_noobj),     list(range(1,5)))#(none,)
...
#####LAYERS
##dense block
def DenseBlock(name,x,num_block=6,growth_rate=32,norm='group_norm',activate='selu',is_training=True):
...
def PrimaryConv(name,x,num_filters=24,norm='group_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        ...
        '''
        x = _conv_block('conv_0',x,num_filters//2,3,2,'SAME',norm,activate,is_training)#none,none/2,none/2,num_filters//2
        x = crelu(x)#none,none/2,none/2,num_filters
        _x = tf.nn.max_pool(x,[1,2,2,1],[1,2,2,1],'SAME')#none,none/4,none/4,num_filters
        x = _conv_block('conv_1',x,int(num_filters/2)//2,1,1,'SAME',norm,activate,is_training)#none,none/2,none/2,num_filters//4
        x = crelu(x)#none,none/2,none/2,num_filters//2
        x = _conv_block('conv_2',x,num_filters//2,3,2,'SAME',norm,activate,is_training)#none,none/4,none/4,num_filters//2
        x = crelu(x)#none,none/2,none/2,num_filters
        x = tf.concat([_x,x],axis=-1)#none,none/4,none/4,2*num_filters
        x = _conv_block('conv_3',x,num_filters//2,1,1,'SAME',None,None,is_training)#none,none/4,none/4,num_filters//2
        x = crelu(x)#none,none/4,none/4,num_filters
        '''
        return x
##_B_conv_block
