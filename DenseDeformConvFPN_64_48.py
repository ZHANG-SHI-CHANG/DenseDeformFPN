...
def PrimaryConv(name,x,num_filters=24,norm='group_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        ...
        x = _conv_block('conv_1',x,int(num_filters/2),1,1,'SAME',norm,activate,is_training)#none,none/2,none/2,num_filters/2
...
def Attention(name,x1,x2):
    with tf.variable_scope(name):
        ...
        _x1 = tf.image.resize_bilinear(x1,[num_x2_0123[1],num_x2_0123[2]])#none,none,none,C1
        w1 = GetWeight('w1',[1,1,C1,C1])
        weight_x1 = tf.nn.conv2d(_x1,w1,[1,1,1,1],'SAME',name='weight_x1')#none,none,none,C1
        ...
        _x1 = tf.expand_dims(_x1,axis=-1)#none,none,none,C1,1
        _x1 = tf.squeeze(tf.matmul(weight_x1,_x1),4)#none,none,none,1
        weight_x2 = tf.expand_dims(weight_x2,axis=3)#none,none,none,1,C2
        _x2 = tf.expand_dims(x2,axis=-1)#none,none,none,C2,1
        _x2 = tf.squeeze(tf.matmul(weight_x2,_x2),4)#none,none,none,1
        bias = tf.get_variable('bias',1,tf.float32,initializer=tf.constant_initializer(0.001))
        
        weight = _x1+_x2+bias#none,none,none,1
